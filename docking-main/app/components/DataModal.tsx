...
export default function DataModal(props: ModalProps) {
    ...
    const buttonClass = loading ? 'bg-gray-400' : 'bg-[#1a1a64]';
    return (
        <>
            <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
                <div className="relative w-auto my-6 mx-auto max-w-3xl">
                    ...
                    <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                        ...
                        {/*body*/}
                        {(response.message.length > 1 && response.ok) ? <p className="relative p-6 flex-auto">Your product has been added successfully</p> : (response.message.length && !response.ok) ? <p className="relative p-6 flex-auto">{response.message}</p> : (
                            <>
                                ...
                                <form className="w-full max-w-lg px-4" onSubmit={handleSubmit}>
                                    ...
                                    <div className="flex items-center justify-end p-6 border-t border-solid border-slate-200 rounded-b">
                                        ...
                                        <button type='submit' className={`${buttonClass}  text-white active:bg-[#1a1a1a] font-bold uppercase text-sm px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150`} disabled={loading}>{buttonContent}</button>
                                    </div>
                                </form>
                            </>)}
                    </div>
